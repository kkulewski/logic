...
fact {
  ...
  // a) named stations are on exactly the lines as shown in graphic
  Stanmore in (JubileeStation - CircleStation - CentralStation)
  BakerStreet in (JubileeStation & CircleStation - CentralStation)
  Epping in (CentralStation - JubileeStation - CircleStation)

  // b) no station (including those unnamed) is on all three lines
  no s: Station | s in (JubileeStation & CircleStation & CentralStation)

  // c) the Circle line forms a circle
  //all cs: CircleStation | one s: CircleStation |  cs in s.^circle
  all cs: CircleStation {
	one cs.circle
	CircleStation in cs.^circle
  }
  ...
  // e) there's a station between Stanmore and BakerStreet
  some s: JubileeStation {
    s not in Stanmore
    s not in BakerStreet
    Stanmore.jubilee = s
    s.jubilee = BakerStreet
  }

  // f) it is possible to travel from BakerStreet to Epping
  Epping in BakerStreet.^(jubilee + circle + central)

}

pred show {}
run show for 5 Station
